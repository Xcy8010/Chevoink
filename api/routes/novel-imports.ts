...
import { novelImportCommitSchema, novelImportConfirmSchema, novelImportCreateSchema, novelImportIntentConfirmSchema } from '../../shared/contracts/novel-import.js'
...
router.use((req, res, next) => {
  try {
    ...
    if (mutation && !cancellation && !novelImportCapabilities().enabled) throw new DataAccessError(503, 'IMPORT_DISABLED', '作品导入尚未开放；仍可查看、下载原文件或取消已有任务。')
...
router.use((req, res, next) => {
  ...
  return (req.method === 'PATCH' && /^\/[^/]+\/manifest$/.test(req.path) ? manifestJson : smallJson)(req, res, next)
})
...
router.get('/capabilities', route(() => novelImportCapabilities()))
router.get('/', route((_req, human) => listNovelImports(human)))
router.post('/preflight', route((_req, human) => preflightNovelImport(human)))
...
router.get('/:jobId/preview', route((req, human) => getNovelImportPreview(human, id(req))))
...
router.post('/:jobId/restore-preview', route((req, human) => previewNovelImportRestore(human, id(req))))
router.post('/:jobId/restore', route((req, human) => restoreNovelImport(human, id(req), z.object({ restoreApprovalId: idSchema, targetHash: z.string().regex(/^[a-f0-9]{64}$/) }).strict().parse(req.body))))
