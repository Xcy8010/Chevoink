...
export const novelImportTool = defineTool({
  name: 'novel_import',
  title: '一键导入',
  description: '把本轮用户上传的原文件导入当前作品：prepare 使用附件真实 URL，持久保存等待位置并打开人类导入面板；已有章节必须先两次独立覆盖确认，再核对具体预览。status 查询任务；commit 只接受 jobId，并仅消费服务端已有、未过期且绑定原文/预览/目标的真实人类批准，不能用 confirmed、通用工具允许或模型参数批准。等待期间不轮询模型、不调用其他写工具绕过确认。仅真实成功回执代表导入完成，不自动发布、不调用付费识别。',
  ...
  async execute(ctx, args) {
    ...
    // No legacy or inline-subagent fallback may bypass the persisted cursor.
