...
export type RoutedImportPlan = { title: string; content: string }
export type RoutedImportMemory = { memoryType: 'characterCard' | 'worldbuilding' | 'storyBible'; title: string; content: string }
    ...
    for (const chapter of volume.chapters) {
      ...
      if (chapter.content.trim()) {
        if (PLAN_TITLE.test(title)) {
          ...
          plans.push({ title, content: chapter.content })
          continue
        }
