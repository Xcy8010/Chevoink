...
export function ImportIntegrityReport({ novelId, jobId, report, disabled, onReview }: {
  ...
  const [page, setPage] = useState(0), [kind, setKind] = useState('all')
  ...
  const items = useMemo(() => report.items.filter(item => kind === 'all' || item.kind === kind), [kind, report.items])
  ...
  return <section className="min-w-0 space-y-3 rounded-xl border border-[var(--border-subtle)] p-3" aria-label="完整性报告">
    ...
    <p className="text-sm">{report.partialImport ? '部分导入：包含用户明确排除的来源，不代表整份原稿完整导入。' : '未标记部分导入；仍须处理所有阻断项。'}处理覆盖率不等于 OCR 文字准确率。</p>
    ...
    <label className="block text-sm">筛选来源类型<select className="ml-2 min-h-11 rounded border bg-[var(--surface-default)] px-2" value={kind} onChange={event => { setKind(event.target.value); setPage(0) }}>{[['all', '全部'], ['file', '文件成员'], ['page', '页面'], ['block', '正文块'], ['region', '区域'], ['image', '图片']].map(([value, text]) => <option key={value} value={value}>{text}</option>)}</select></label>
    <div className="max-h-[60dvh] space-y-3 overflow-y-auto">{items.slice(page * 30, page * 30 + 30).map(item => {
      ...
      const canReview = issuesByItem.get(item.id)?.some(issue => !issue.resolved && issue.resolution === 'review') ?? false
