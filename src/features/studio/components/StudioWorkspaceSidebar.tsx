...
export default function StudioWorkspaceSidebar(props: Props) {
  ...
  function openContextAt(trigger: HTMLElement, target: ContextTargetInput) {
  ...
  return <>
    ...
    {context ? <div onMouseDown={(event) => event.stopPropagation()} className="fixed z-[190] max-h-[calc(100dvh-24px)] w-48 overflow-y-auto rounded-[12px] border border-[var(--border-subtle)] bg-[var(--surface-default)] p-1 shadow-2xl" style={{ left: Math.min(context.x, window.innerWidth - 208), top: Math.max(12, Math.min(context.y, window.innerHeight - (context.kind === 'novel' ? 424 : 304))) }}>
      ...
      {context.kind === 'novel' && context.id === props.currentNovelId ? <><div className={menuDivider} />{props.onOpenNovelMeta ? <button type="button" onClick={() => { setContext(null); props.onOpenNovelMeta?.() }} className={menuItem}><Settings2 className="h-3.5 w-3.5" />作品设置</button> : null}{props.onPublishNovel ? <button type="button" onClick={() => { setContext(null); props.onPublishNovel?.() }} className={menuItem}><Upload className="h-3.5 w-3.5" />{publishLabel}</button> : null}{props.onToggleNovelCompletion && props.currentNovelStatus ? <button type="button" onClick={() => { setContext(null); props.onToggleNovelCompletion?.() }} className={menuItem}>{novelCompleted ? <RotateCcw className="h-3.5 w-3.5" /> : <Flag className="h-3.5 w-3.5" />}{novelCompleted ? '继续连载' : '完结作品'}</button> : null}{props.onImportNovel ? <button type="button" onClick={() => { contextTriggerRef.current?.focus(); setContext(null); props.onImportNovel?.() }} className={cn(menuItem, 'min-h-11')}><Upload className="h-3.5 w-3.5" />一键导入</button> : null}{props.onExportNovel ? <button type="button" onClick={() => { setContext(null); props.onExportNovel?.() }} className={menuItem}><FolderDown className="h-3.5 w-3.5" />一键导出</button> : null}</> : null}
      {context.kind === 'novel' && context.id === props.currentNovelId && props.onImportHistory ? <button type="button" onClick={() => { contextTriggerRef.current?.focus(); setContext(null); props.onImportHistory?.() }} className={cn(menuItem, 'min-h-11')}><RotateCcw className="h-3.5 w-3.5" />导入记录与恢复</button> : null}
