import type { NovelImportCapabilities, NovelImportJobStatus, NovelImportManifestEdit, NovelImportModelSelection, NovelImportPreflight, NovelImportPreview, NovelImportReceipt } from '../../../shared/contracts/novel-import.js'
...
export const novelImportApi = {
  ...
  preflight: (novelId: string) => requestData<NovelImportPreflight>(`${root(novelId)}/preflight`, json('POST')),
  ...
  attachment: (novelId: string, jobId: string, attachment: { url: string; runId: string }) => requestData<NovelImportJobStatus>(`${jobPath(novelId, jobId)}/attachment`, json('POST', attachment)),
  ...
  confirm: (novelId: string, job: NovelImportJobStatus, preview: NovelImportPreview) => requestData<{ approvalId: string; expiresAt: string }>(`${jobPath(novelId, job.jobId)}/confirm`, json('POST', { manifestRevision: preview.manifestRevision, manifestHash: preview.manifestHash, targetHash: job.targetHash })),
  ...
  restorePreview: (novelId: string, jobId: string) => requestData<{ restoreApprovalId: string; targetHash: string; expiresAt: string }>(`${jobPath(novelId, jobId)}/restore-preview`, json('POST')),
  restore: (novelId: string, jobId: string, approval: { restoreApprovalId: string; targetHash: string }) => requestData<NovelImportReceipt & { restored: true }>(`${jobPath(novelId, jobId)}/restore`, json('POST', { restoreApprovalId: approval.restoreApprovalId, targetHash: approval.targetHash })),
}
